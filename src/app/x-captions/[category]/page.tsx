 ...
 * @module app/x-captions/[category]/page
 */
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  /** 使用 SEO 配置生成元数据 */
  const seoConfig = generateCategoryPageSeo(category.displayName, category.slug, 'x');
  
  /** 规范链接 URL */
  const canonicalUrl = `${SITE_CONFIG.url}/x-captions/${category.slug}`;
      ...
      images: [
        {
          ...
          alt: `${category.displayName} X Captions`,
        },
      ],
    },
    twitter: {
...
export default async function XCategoryPage({ params }: PageProps) {
  ...
  const breadcrumbItems: SeoBreakcrumbItem[] = [
    { label: 'Home', href: '/' },
    { label: 'X Captions', href: '/x-captions' },
  ...
  const jsonLdBreadcrumbs = [
    ...
    { name: category.displayName, url: `${SITE_CONFIG.url}/x-captions/${category.slug}` },
  ];
  ...
  return (
    <div className="flex flex-col">
      ...
      <CategoryPageJsonLd
        ...
        platform="X (Twitter)"
        title={`X ${category.displayName} Captions`}
        description={`Best ${category.displayName.toLowerCase()} captions for X (Twitter)`}
        url={`${SITE_CONFIG.url}/x-captions/${category.slug}`}
        siteUrl={SITE_CONFIG.url}
        parentUrl={`${SITE_CONFIG.url}/x-captions`}
